...
`SERVICE_ID`는 포탈 `services.id`와 **정확히 일치**해야 한다(대소문자·하이픈까지).
...
const SERVICE_ID = 'my-service'      // ← 포탈 services.id 와 정확히 일치
...
| `sso_failed` | 실제 검증 실패 (키·SERVICE_ID·tenant 불일치) | 포탈 `?error=sso_failed` (관리자 로깅) |
| `sso_not_issued` | 키 미승인 (포탈이 진입 전 차단·발급 — 이 서비스는 emit·handle 안 함) | 관리자 승인 대기 안내 (코드 수정 대상 아님) |

에러가 보이면 **코드를 다시 고치기 전에 포탈부터 확인**하도록 안내:
`sso_failed` → `SSO_SECRET` 불일치 / `SERVICE_ID` ≠ `services.id` / "SSO 연동" 체크 누락(토큰 미수신).
