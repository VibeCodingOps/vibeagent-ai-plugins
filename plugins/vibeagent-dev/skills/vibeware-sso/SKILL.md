...
VibeWare 포탈 등록/키 발급은 **AI가 대신 못 한다**(포탈 브라우저 로그인 + 별도 관리자 승인). 그리고 **`SSO_SECRET` 같은 서버 서명키는 브리지가 내려주지 않는다**(브리지가 주는 건 Supabase 값뿐) — 사용자가 포탈에서 발급받아 직접 줘야 한다.
...
`SSO_SECRET`은 브리지 도구로 자동으로 못 받는다 — 브리지가 내려주는 건 Supabase 값(`get_public_env`의 anon key·URL, 편집자/소유자의 `get_service_role_key`)뿐이고 SSO 키는 대상이 아니다. 사용자가 4단계에서 복사한 키를 받아 넣는다 — **두 경로**:
